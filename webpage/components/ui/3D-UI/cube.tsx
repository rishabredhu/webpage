    ...
    function HilbertCurve() {
      ...
      useEffect(() => {
        if (curveRef.current) {
          ...
          const points = GeometryUtils.hilbert3D(new THREE.Vector3(0, 0, 0), size, iterations, 0, 1, 2, 3, 4, 5, 6, 7)
    ...
    function Sparkles() {
      ...
      useFrame(({ clock }) => {
        if (sparklesRef.current) {
          ...
          for (let i = 0; i < SPARKLE_COUNT; i++) {
            const i3 = i * 3
            positions[i3 + 1] = (positions[i3 + 1] + CUBOID_SIZE.y / 2) % CUBOID_SIZE.y - CUBOID_SIZE.y / 2
    ...
    function Module({ modelPath, position }: { modelPath: string, position: THREE.Vector3 }) {
