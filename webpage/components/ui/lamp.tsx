...
const LampPost = ({ rotationY }: { rotationY: number }) => {
  ...
  useFrame((state) => {
    if (modelRef.current) {
      modelRef.current.position.y = Math.sin(state.clock.getElapsedTime()) * 0.1 - 0.8; // Adjust height here
...
const Lamp = ({ rotationY = 180 }: { rotationY?: number }) => {
  return (
    ...
    >
      ...
      <OrbitControls enableZoom={false} enableRotate={true} /> {/* Disables zoom and rotation */}
    </Canvas>
  );
};
